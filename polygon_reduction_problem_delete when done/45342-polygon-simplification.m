...
vp = 1 + mod(v, numv); %RICH just a standard mod no matrix
vm = 1 + mod(v - 2, numv);

% Obtain adjacent line segments and their lengths
dir1 = poly(:,v) - poly(:,vm); %direction1!!
dir2 = poly(:,vp) - poly(:,v); %direction2
len1 = norm(dir1); %RICH, literally just length of line
len2 = norm(dir2);
...
len1len2 = len1 * len2;
a = abs(acos((dir1' * dir2) / len1len2)) * len1len2; %RICH, FOUND EXAMPLE WITH NO arccos2 here: http://onlinemschool.com/math/library/vector/angl/
