...
% Find adjacent vertices    ****works out the actual vertex positions (id)
vp = 1 + mod(v, numv);
vm = 1 + mod(v - 2, numv);

% Obtain adjacent line segments and their lengths   ****the ":" means "all", this is essentially 
dir1 = poly(:,v) - poly(:,vm);     **** i think they are still 2d vectors here, so it's the length as a vector
dir2 = poly(:,vp) - poly(:,v);
len1 = norm(dir1);    **** this is like when you square root it
len2 = norm(dir2);          *** the importance is based on these two lengths multiplied
...
len1len2 = len1 * len2;
a = abs(acos((dir1' * dir2) / len1len2)) * len1len2;   ****arcos is for vectors, we are getting the angle out
%a = abs(1 - ((dir1' * dir2) / len1len2)) * len1len2;    ***UNSURE IS COMMENTED
