...
print('loading generalmath from https://love2d.org/wiki/General_math')
...
-- there is some discussion at http://lua-users.org/wiki/SimpleRound...
-- however this one seems to round negatives fine:
-- math.round(-12562.5678, 2) => -12562.57
...
function math.angle_to_vector(angle) --take an angle in radians to a vector2
    --this one means 0° is East (positive x), 90° is South (positive y)
    --an alternative was (sin,-cos)
    --however this is just a correction to make the angle go N at 0°
    --my code is the normal way, ref: https://en.wikipedia.org/wiki/Parametric_equation
    return math.cos(angle), math.sin(angle)
end

function math.pos_sin(x)
    --sine function where we range from 0 to 1, instead of -1 to 1
    return (math.sin(x) + 1) / 2
end

function math.dotprod(a, b) --haven't actually used this yet, it was going to solve poly reduction
    local ret = 0
    for i = 1, #a do
        ret = ret + a[i] * b[i]
    end
...
--THESE ARE LIKELY TRASH (from old stuff)
-- function math.vector_to_angle(x, y) return math.atan2(x, -y) end --i think this might be wrong with the minus
